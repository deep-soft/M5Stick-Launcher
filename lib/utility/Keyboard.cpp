...
uint8_t Keyboard_Class::getKey(Point2D_t keyCoor) {
    ...
    if ((keyCoor.x < 0) || (keyCoor.y < 0)) { return 0; }
...
uint8_t Keyboard_Class::isPressed() { return _key_list_buffer.size(); }
...
void Keyboard_Class::updateKeysState() {
    ...
    for (auto &i : _key_list_buffer) {
        ...
        if (getKeyValue(i).value_first == ' ') { _keys_state_buffer.space = true; }
        _key_pos_hid_keys.push_back(i);
    ...
    for (auto &k : _keys_state_buffer.modifier_keys) { _keys_state_buffer.modifiers |= (1 << (k - 0x80)); }
    ...
    for (auto &i : _key_pos_hid_keys) {
        ...
        if (key) { _keys_state_buffer.hid_keys.push_back(key); }
    }
